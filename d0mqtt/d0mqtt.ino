...
//
// For IR repeater mode:
//  SIR 204 EVL infrared LED, 875 nm, 30 deg, 3 mm, 1.3 V Ufwd
//   => series R = (3.3V - 1.3V) / 10 mA ~= 2/0.010 ~= 220 ohm .. 330 ohm
//   + SIR204 cathode (flattened side) via 330 ohm to Pin D2
//   + SIR204 anode (round side of plastic) to 3.3V
//
...
#define HAVE_IR_REPEATER_LED // define to repeat the received IR data on an output IR LED
...
int readSMLFrame(SoftwareSerial* ss, byte* frame, unsigned int bufsize, unsigned int* noverflow = NULL)
{
  ...
  enum {
    ...
    STATE_READ_FRAME,          // read everything until reoccurrence of [1B][1B][1B][1B]
...
#ifndef HAVE_IR_REPEATER_LED
  d0.begin(9600, SWSERIAL_8N1, D1, -1, /*invert:*/false, 512);
  d0.enableTx(false);
#else
  d0.begin(9600, SWSERIAL_8N1, D1, D2, /*invert:*/false, 512);
  d0.enableTx(true);
#endif
...
#ifdef HAVE_IR_REPEATER_LED
  Serial.println(F("Repeating the data via IR LED in series with 330 ohm from Pin D2 to Vcc, hi=dark, lo=light"));
#endif
...
void loop()
{
  ...
  for (int ntx = 0; ntx < nrx; ntx++) {
    d0.write(smlframe[ntx]);
  }
#endif
